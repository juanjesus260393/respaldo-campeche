...
//Modulo que se encarga de gestionar la informacion de los lugares en base  alos parametros que recibe
    public static function search_places() {
        if (isset($_GET["sector_id"])) {
            ...
            $dbh = Conectar::con();
            //Consulta que obtiene los sitios de un sector en especifico
...
inner join descripcion_idioma d on i.id_revision_informacion= d.id_revision_informacion where d.lang_code = 'ES' and e.id_sector = '$sector_id' and i.status = 'A' order by s.id_sitio;";
            ...
            //Si no se tiene un registro que coincida con los parametros  de la consulta se regresa un arreglo nulo
            if ($sectores == null) {
                $sectores = $sectores2;
            }
            return $sectores;
        } else {
            //En caso de que no exista el identificador del sector se envia un encabezado 400
            header("HTTP/1.0 400 Bad Request");
            die();
        }
    }

//Funcion que se encarga de gestionar la informacion de los lugares que tienen cupones asociados
    public static function search_places_with_cupons() {
        if (isset($_GET["sector_id"])) {
            ...
            $dbh = Conectar::con();
            $fa = date('Y-m-d');
            //Consulta que obtiene la informacion de sitios que tienen cupones asociados y que esten vigentes
...
group by e.id_empresa;";
            ...
            //Si no se recibe el identificador del sector se envia un encabezado 400
            header("HTTP/1.0 400 Bad Request");
            die();
        }
    }
