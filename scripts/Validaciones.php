...
//Obtener cantidad de registros en base a una empresa
                ...
                } else if ($hoy2 > $fechafin) {
                    echo '<script language = javascript> alert("Membresia Caducada") </script>';
...
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
                $result = $mysqli->query("select c.id_cupon from (cupon c inner join revision_objeto r on c.id_revision_objeto = r.id_revision_objeto)"
                        ...
                        . "AND r.fecha_creacion>='" . $FI->format('Y-m-d') . "' AND r.fecha_creacion<='" . $FT->format('Y-m-d') . "' group by r.id_revision_objeto");
...
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
        ...
        public function registros_vacante() {
            $mysqli = Conectar::con();
            if (mysqli_connect_errno()) {
                printf("Conexión fallida: %s\n", mysqli_connect_error());
                exit();
            ...
            if ($_SESSION['id_membresia'] == "3") {
//Obtener cantidad de Vacantes en base a una empresa
                $nrpm = '3';
//Obtener la cantidad de Registros entre el registro de la primera vacante y la fecha de termino del mes actual
                $result = $mysqli->query("select c.id_vacante from (vacante c inner join revision_objeto r on c.id_revision_objeto = r.id_revision_objeto) inner join empresa e on e.id_empresa = r.id_empresa where e.id_empresa = " . $_SESSION['idemp'] . " group by r.id_revision_objeto");
                $row_cnt = $result->num_rows;
                $nrbd = $row_cnt;
                if ($nrbd >= $nrpm) {
                    echo '<script language = javascript> alert("Solo puedes subir 3 Vacantes, si deseas agregar otra, primero debes elimina alguna Vacante de los que tienes registradas") </script>';
                    //Regresamos a la pagina anterior
                    echo "<html><head></head>" .
                    "<body onload=\"javascript:history.back()\">" .
                    "</body></html>";
...
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
                ...
                $nrbd = $row_cnt;
                if ($nrbd >= $nrpm) {
                    echo '<script language = javascript> alert("Solo puedes subir 2 Vacantes, si deseas agregar otra, primero debes elimina alguna Vacante de los que tienes registradas") </script>';
                    //Regresamos a la pagina anterior
                    echo "<html><head></head>" .
                    "<body onload=\"javascript:history.back()\">" .
                    "</body></html>";
...
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
                ...
                $nrbd = $row_cnt;
                if ($nrbd >= $nrpm) {
                    echo '<script language = javascript> alert("Para Agregar Vacantes necesitas cambiar de membresia") </script>';
                    //Regresamos a la pagina anterior
                    echo "<html><head></head>" .
                    "<body onload=\"javascript:history.back()\">" .
                    "</body></html>";
        ...
        public function registros_evento() {
            $mysqli = Conectar::con();
            if (mysqli_connect_errno()) {
                printf("Conexión fallida: %s\n", mysqli_connect_error());
                exit();
...
//Obtener cantidad de registros en base a una empresa
                ...
                $c = "FB";
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
                $resultF = $mysqli->query("select a.id_ad, a.id_revision_objeto, a.tipo, a.id_img, revision_objeto.status "
                        . "from (ad a inner join revision_objeto on a.id_revision_objeto = revision_objeto.id_revision_objeto) "
                        . "inner join empresa on revision_objeto.id_empresa = " . $_SESSION['idemp'] . "  "
                        . "WHERE revision_objeto.fecha_creacion>='" . $FI->format('Y-m-d') . "' "
                        . "AND revision_objeto.fecha_creacion<='" . $FT->format('Y-m-d') . "' AND a.tipo='F' group by id_ad;");
                $row_cntF = $resultF->num_rows;
                $nrbdF = $row_cntF;

                if ($nrbdF >= $nrpmF) {

                    $a = "NF";
                    echo '<script language = javascript> alert("Solo puedes subir 2 Flyers , si deseas agregar otro, primero elimina alguno de los que tienes registrados") </script>';
                    //Regresamos a la pagina anterior
                    /*   echo "<html><head></head>" .
                      "<body onload=\"javascript:history.back()\">" .
                      "</body></html>";
                ...
                $resultB = $mysqli->query("select a.id_ad, a.id_revision_objeto, a.tipo, a.id_img, revision_objeto.status "
                        . "from (ad a inner join revision_objeto on a.id_revision_objeto = revision_objeto.id_revision_objeto) "
                        . "inner join empresa on revision_objeto.id_empresa = " . $_SESSION['idemp'] . "  "
                        . "WHERE revision_objeto.fecha_creacion>='" . $FI->format('Y-m-d') . "' "
                        . "AND revision_objeto.fecha_creacion<='" . $FT->format('Y-m-d') . "' AND a.tipo='B' group by id_ad;");
                $row_cntB = $resultB->num_rows;
                $nrbdB = $row_cntB;
                if ($nrbdB >= $nrpmB) {
                    echo '<script language = javascript> alert("Solo puedes subir 2 Banners, si deseas agregar otro, primero elimina alguno de los que tienes registrados") </script>';
                    $b = "NB";
                }
...
//Obtener la cantidad de Registros entre el registro del primer cupon y la fecha de termino del mes actual
                ...
                if ($nrbdF >= $nrpmF) {
                    $a = "NF";
                    echo '<script language = javascript> alert("Solo puedes subir 1 flyers, si deseas agregar otro, primero elimina alguno de los que tienes registrados") </script>';
                }
                ...
                if ($nrbdB >= $nrpmB) {
                    $b = "NB";
                    echo '<script language = javascript> alert("Solo puedes subir 1 Banners, si deseas agregar otro, primero elimina alguno de los que tienes registrados") </script>';
                }
